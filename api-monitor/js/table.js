...
async function loadEndpointsTable(tbody) {
    try {
        const data = await fetchWithRetry('/api/endpoints');
        const endpoints = data.endpoints || [];

        if (endpoints.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" class="text-center text-muted">No endpoints found.</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = endpoints.map(api => {
            ...
            let methodHtml = '';
            if (api.method === 'GET') {
                methodHtml = `<span class="badge badge-blue">GET</span>`;
            } else if (api.method === 'POST') {
                methodHtml = `<span class="badge badge-purple">POST</span>`;
            } else if (api.method === 'PUT') {
                methodHtml = `<span class="badge" style="background-color: rgba(245, 158, 11, 0.1); color: #F59E0B;">PUT</span>`;
            } else if (api.method === 'DELETE') {
                methodHtml = `<span class="badge badge-danger">DELETE</span>`;
            } else {
                methodHtml = `<span class="badge badge-gray">${api.method}</span>`;
            }
        ...
        tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger">Error loading endpoints.</td></tr>`;
    }
}
        ...
        tbody.innerHTML = employees.map((user, index) => {
            ...
            return `
                <tr>
                    ...
                    <td class="text-right">
                        <div class="action-btns">
                            <button class="action-btn" title="Edit Credentials" onclick="alert('Cred ID: ${escapeHtml(user.employee_id)}')"><i class="fa-solid fa-key"></i></button>
                            <button class="action-btn delete" title="Suspend"><i class="fa-solid fa-user-xmark"></i></button>
                        </div>
                    </td>
                </tr>
            `;
